...
const VolunteeringDetails = ({ route, navigation }) => {
  ...
  // עדכון לפונקציית handleApply בקובץ VolunteeringDetails.js
  const handleApply = async () => {
    ...
    try {
      ...
      // בדיקה חדשה - האם המשימה כבר הוצעה למתנדב
      ...
      if (volunteerProfileDoc.exists()) {
        ...
        // בדיקה אם יש מערך offeredTasks
        if (volunteerData.offeredTasks && Array.isArray(volunteerData.offeredTasks)) {
          // בדיקה אם המשימה קיימת במערך offeredTasks
          const isAlreadyOffered = volunteerData.offeredTasks.some(
            task => task.id === volunteering.id
          );
          
          if (isAlreadyOffered) {
            Alert.alert(
              'הודעה', 
              'התנדבות זו כבר הוצעה לך ישירות. אנא בדוק את ההצעות שלך במסך הפרופיל.'
            );
            return;
          }
        }
      ...
      // חילוץ organizerId מ-volunteeringId (בהנחה שהפורמט הוא "organizerId_timestamp")
      let organizerId = volunteering.id;
      if (volunteering.id.includes('_')) {
        organizerId = volunteering.id.split('_')[0];
      }
      
      console.log("Extracted organizerId:", organizerId);
