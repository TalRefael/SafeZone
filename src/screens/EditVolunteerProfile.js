...
const EditVolunteerProfile = ({ route, navigation }) => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (!userId) {
          ...
          Alert.alert("שגיאה", "מזהה משתמש לא תקין");
          setLoading(false);
          navigation.goBack();
          return;
        }
        
        // שימוש בuserId ישירות
        ...
        if (volunteerProfileDoc.exists()) {
          ...
          console.log("No volunteer profile found - creating new");
          Alert.alert(
            "מידע חסר", 
            "לא נמצא פרופיל מתנדב קיים. ניתן ליצור פרופיל חדש.",
            [
              { text: "ביטול", onPress: () => navigation.goBack() },
        ...
        Alert.alert("שגיאה", `שגיאה בטעינת נתונים: ${error.message}`);
        setLoading(false);
        navigation.goBack();
      }
    };
  ...
  const updateVolunteerProfile = async () => {
    try {
      ...
      // בדיקת תקינות userId
      if (!userId) {
        Alert.alert('שגיאה', 'אין מידע תקין על המשתמש');
        setIsSaving(false);
        return;
      }
      ...
      if (profilePicture && (profilePicture.startsWith('file:') || profilePicture.startsWith('content:'))) {
        ...
        try {
          ...
          Alert.alert("אזהרה", "התמונה לא הועלתה בהצלחה, אך שאר הפרופיל יישמר");
      ...
      console.log("Updating profile data with picture URL:", profilePictureUrl);
      
      // שמירת פרופיל המתנדב
      const volunteerProfileRef = doc(db, 'volunteerProfiles', userId);
      await setDoc(volunteerProfileRef, volunteerProfileData, { merge: true });
      
      setIsSaving(false);
      Alert.alert(
        'הצלחה',
        'פרופיל המתנדב עודכן בהצלחה!',
        [{ text: 'אישור', onPress: () => navigation.navigate('HomePage') }]
      );
    } catch (error) {
      console.error('Error updating volunteer profile:', error.message, error.stack);
      Alert.alert("שגיאה", `שגיאה בעדכון פרופיל המתנדב: ${error.message}`);
      setIsSaving(false);
    }
  };
  ...
  const pickImage = async () => {
    try {
      ...
      if (!permissionResult.granted) {
        Alert.alert('הרשאות חסרות', 'נדרשת הרשאה לגישה לגלריה כדי לבחור תמונת פרופיל');
        return;
      }
      ...
      Alert.alert(`שגיאה בבחירת תמונה: ${error.message}`);
    }
  };
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>טוען נתוני פרופיל...</Text>
      </View>
    );
  }
    ...
    >
      ...
      >
          ...
          >
            ...
            <View style={styles.switchContainer}>
              ...
              <Switch
                ...
                thumbColor={hasCar ? "#2c6975" : "#f4f3f4"}
              />
            </View>
        ...
        >
          <TouchableOpacity 
            style={styles.button} 
            onPress={updateVolunteerProfile}
            disabled={isSaving}
          >
            {isSaving ? (
